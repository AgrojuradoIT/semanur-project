...
class ChecklistFormScreen extends StatefulWidget {
  ...
  const ChecklistFormScreen({super.key, required this.vehiculo});
...
class _ChecklistFormScreenState extends State<ChecklistFormScreen> {
  ...
  Future<void> _loadUsers() async {
    // Cargar usuarios via Provider (quien maneja API + Cache local)
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<UserProvider>().fetchUsers(
        role: 'operador',
      ); // Opcional filtrar rol
    });
  }
  ...
  Widget _buildOperatorAssignment() {
    ...
    return DropdownButtonFormField<int>(
      key: ValueKey(_selectedOperatorId), // Force rebuild when ID changes
      decoration: const InputDecoration(
        labelText: 'Asignar Conductor / Operador',
        border: OutlineInputBorder(),
      ...
      validator: (val) => val == null ? 'Debe asignar un conductor' : null,
    );
  }
  ...
  void _submit() async {
    if (_isAerial) {
      ...
      // But we still validate operator assignment if applicable
    ...
    // Save operator assignment locally
    if (_selectedOperatorId != null) {
      try {
        await DatabaseHelper().updateVehicleOperator(
          widget.vehiculo.id,
          _selectedOperatorId!,
        );
      } catch (e) {
        debugPrint('Error updating vehicle operator locally: $e');
      }
    }
